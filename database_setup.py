...
TABLE_STATEMENTS = [
	...
	CREATE TABLE IF NOT EXISTS pd_deliveries (
		...
		order_plan_id BIGINT DEFAULT NULL COMMENT '关联订货计划ID（与报单人、合同报货计划对应）',
		...
		INDEX idx_order_plan_id (order_plan_id),
	...
	CREATE TABLE IF NOT EXISTS pd_weighbills (
		...
		is_last_truck_for_order_plan TINYINT DEFAULT 0 COMMENT '是否为订货计划最后一车',
	...
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-品类字典表';
	""",
	"""
	CREATE TABLE IF NOT EXISTS dict_warehouses (
		...
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-仓库字典表';
	""",
	"""
	CREATE TABLE IF NOT EXISTS dict_factories (
		...
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-冶炼厂字典表';
	""",
	"""
	CREATE TABLE IF NOT EXISTS freight_rates (
		...
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		CONSTRAINT fk_tl_freight_factory FOREIGN KEY (factory_id) REFERENCES dict_factories (id) ON UPDATE CASCADE ON DELETE RESTRICT,
		CONSTRAINT fk_tl_freight_warehouse FOREIGN KEY (warehouse_id) REFERENCES dict_warehouses (id) ON UPDATE CASCADE ON DELETE RESTRICT,
		UNIQUE KEY uk_tl_factory_warehouse_date (factory_id, warehouse_id, effective_date),
		INDEX idx_tl_effective_date (effective_date)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-运费价格表';
	""",
	"""
	CREATE TABLE IF NOT EXISTS quote_table_metadata (
		...
		execution_date VARCHAR(50) COMMENT '执行日期',
		...
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		CONSTRAINT fk_tl_metadata_factory FOREIGN KEY (factory_id) REFERENCES dict_factories (id) ON UPDATE CASCADE ON DELETE RESTRICT,
		UNIQUE KEY uk_tl_factory_quote_date (factory_id, quote_date)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-报价表元数据';
	""",
	"""
	CREATE TABLE IF NOT EXISTS factory_tax_rates (
		...
		tax_rate DECIMAL(6, 4) NOT NULL COMMENT '税率值',
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		CONSTRAINT fk_tl_tax_factory FOREIGN KEY (factory_id) REFERENCES dict_factories (id) ON UPDATE CASCADE ON DELETE CASCADE,
		UNIQUE KEY uk_tl_factory_tax_type (factory_id, tax_type)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-冶炼厂税率表';
	""",
	"""
	CREATE TABLE IF NOT EXISTS quote_details (
		...
		price_field_sources JSON NULL COMMENT '各价格字段来源',
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		CONSTRAINT fk_tl_detail_factory FOREIGN KEY (factory_id) REFERENCES dict_factories (id) ON UPDATE CASCADE ON DELETE RESTRICT,
		CONSTRAINT fk_tl_detail_metadata FOREIGN KEY (metadata_id) REFERENCES quote_table_metadata (id) ON UPDATE CASCADE ON DELETE SET NULL,
		UNIQUE KEY uk_tl_factory_category_date (factory_id, category_name, quote_date),
		INDEX idx_tl_quote_date (quote_date),
		INDEX idx_tl_factory_id (factory_id),
		INDEX idx_tl_category_name (category_name)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-报价明细表';
	""",
	"""
	CREATE TABLE IF NOT EXISTS warehouse_inventories (
		id INT AUTO_INCREMENT PRIMARY KEY,
		warehouse_id INT NOT NULL COMMENT '仓库ID',
		category_id INT NOT NULL COMMENT '品类行ID',
		available_tons DECIMAL(10, 3) NOT NULL DEFAULT 0 COMMENT '当前可用吨数',
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		CONSTRAINT fk_tl_inv_warehouse FOREIGN KEY (warehouse_id) REFERENCES dict_warehouses (id) ON UPDATE CASCADE ON DELETE RESTRICT,
		CONSTRAINT fk_tl_inv_category FOREIGN KEY (category_id) REFERENCES dict_categories (row_id) ON UPDATE CASCADE ON DELETE RESTRICT,
		UNIQUE KEY uk_tl_inventory_warehouse_category (warehouse_id, category_id)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-仓库库存表（预留）';
	""",
	"""
	CREATE TABLE IF NOT EXISTS factory_demands (
		...
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		CONSTRAINT fk_tl_demand_factory FOREIGN KEY (factory_id) REFERENCES dict_factories (id) ON UPDATE CASCADE ON DELETE RESTRICT,
		UNIQUE KEY uk_tl_factory_demand_date (factory_id, demand_date)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-冶炼厂需求主表（预留）';
	""",
	"""
	CREATE TABLE IF NOT EXISTS factory_demand_items (
		...
		created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		CONSTRAINT fk_tl_ditem_demand FOREIGN KEY (demand_id) REFERENCES factory_demands (id) ON UPDATE CASCADE ON DELETE CASCADE,
		CONSTRAINT fk_tl_ditem_category FOREIGN KEY (category_id) REFERENCES dict_categories (row_id) ON UPDATE CASCADE ON DELETE RESTRICT,
		UNIQUE KEY uk_tl_demand_category (demand_id, category_id)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TL-冶炼厂需求明细表（预留）';
	""",
]
...
def ensure_tl_quote_details_price_field_sources_column():
	"""旧库升级：为 TL quote_details 增加 price_field_sources（新建库已由 CREATE TABLE 包含）。"""
	config = get_mysql_config()
	connection = pymysql.connect(**config)
	try:
		with connection.cursor() as cursor:
			cursor.execute("SHOW TABLES LIKE 'quote_details'")
			if cursor.fetchone() is None:
				return
			cursor.execute(
			...
			if cursor.fetchone()[0] == 0:
				...
				print("quote_details 已添加 price_field_sources 列")
		connection.commit()
	finally:
		connection.close()


def init_tl_default_dict_rows():
	"""TL 比价：插入默认仓库与冶炼厂（与 PD_max 行为一致，便于联调）。"""
	config = get_mysql_config()
	connection = pymysql.connect(**config)
	try:
		with connection.cursor() as cursor:
			...
			if cursor.fetchone() is None:
				return
			cursor.execute(
				"INSERT IGNORE INTO dict_warehouses (id, name, is_active) VALUES (1, '默认仓库', 1)"
			)
			cursor.execute(
				"INSERT IGNORE INTO dict_factories (id, name, is_active) VALUES (1, '默认冶炼厂', 1)"
			)
		connection.commit()
	finally:
		connection.close()
...
def create_tables() -> None:
	...
	try:
		...
		try:
			...
			print(f"检查/添加 quote_details.price_field_sources 失败: {exc}")
		try:
			init_tl_default_dict_rows()
		except Exception as exc:
			print(f"TL 默认字典数据初始化失败: {exc}")
	finally:
		connection.close()
